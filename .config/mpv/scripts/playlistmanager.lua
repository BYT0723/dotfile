...
-- 自定义函数：将一个文件的路径转换为相对于指定目录的路径
function rebase_path(file_path, base_path)
	-- 确保 base_path 以斜杠结尾
	if not base_path:match("/$") then
		base_path = base_path .. "/"
	end

	-- 检查 file_path 是否以 base_path 开头
	if file_path:sub(1, #base_path) == base_path then
		-- 移除 base_path 部分，返回相对路径
		return file_path:sub(#base_path + 1)
	else
		-- 如果不在 base_path 内，返回原路径
		return file_path
	end
end
...
function get_name_from_index(i, notitle)
	...
	if string.sub(name, 1, 1) == "/" or name:match("^%a:[/\\]") then
		name = rebase_path(name, mp.get_property("working-directory"))
	end
